...
class DocumentController extends BaseController
{
    ...
    public function index()
    {
        switch ($this->method) {
            case "HEAD":
            case "GET":
                // If a stateId is passed then redirect to get method
                if (isset($this->params[$this->document_ident])) {
                    return $this->get();
                }
                else {
                    return $this->all();
    ...
    public function getAttachedContent($name = 'content')
    {
        switch ($this->method) {
            case "PUT":
                if ($this->CORS) {
                    // Cross browser PUT implementation, using POST
    ...
    public function validateTimestamp($timestamp)
    {
        ...
        if (!preg_match($regex, $timestamp) > 0) {
            return false;
        }
